    ...
    if (artistName) {
      try {
        ...
        if (liveTracks && liveTracks.length > 0) {
          ...
          // artist_id, so a request supplying a ?name= that doesn't belong to
          // this artistId could store the wrong artist's tracks for everyone.
          // Cross-check the supplied name against the canonical name for this id
          // in the artists table. If a canonical name exists and disagrees,
          // serve the live tracks to this caller but skip the shared write so
          // other users aren't contaminated.
          ...
          const namesMatch =
            !canonical?.name ||
            canonical.name.toLowerCase().trim() === artistName.toLowerCase().trim()
          ...
          if (namesMatch) {
            ...
            console.warn(
              `[tracks] name/id mismatch — skipping shared cache write artistId=${artistId} suppliedName="${artistName}" canonical="${canonical?.name}"`
            )
          }
